...
String& String::insert(size_t pos, const String& str) {
    ...
    String temp = str;
    size_t lengthOfStrToInsert = temp.sz;
    
    char* ptrToBeginning = &cstr[pos];
    char* ptrToEnd = &cstr[sz];
    char* ptrToOnePastEnd = &cstr[sz + 1];
    
    for(size_t i = 0; i < lengthOfStrToInsert; i++) {
        while(ptrToOnePastEnd != ptrToBeginning) {
            *ptrToOnePastEnd = *ptrToEnd;
            --ptrToOnePastEnd;
            --ptrToEnd;
        }
        sz += 1;
        ptrToEnd = &cstr[sz];
        ptrToOnePastEnd = &cstr[sz + 1];
    }
    
    cstr[sz + 1] = a_null_byte;
    
    char const * strToInsert = &temp[0];
    
    for(size_t i = pos; i < pos + lengthOfStrToInsert; i++) {
        *ptrToBeginning = *strToInsert;
        ++ptrToBeginning;
        ++strToInsert;
